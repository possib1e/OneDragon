...
更多运行环境说明见 [Installation Notes](docs/installation.md)，配置整理计划见 [Configuration Notes](docs/configuration.md)。
...
更完整的维护路线见 [Maintenance Roadmap](docs/roadmap.md)。

## 发布记录

- [v0.1.1 Maintenance Release Notes](docs/releases/v0.1.1.md)

## 当前状态

项目保留了早期的一体化工具链结构，当前维护会优先处理文档、运行入口、安全边界、输出目录和配置项，再逐步拆分高风险的硬编码命令。由于扫描流程依赖 masscan、nmap、ffuf、xray、AWVS 等外部工具，建议先在隔离测试环境中验证参数和输出文件，再用于授权项目。

## 历史说明

### 2026.06.02

- 增加安装说明、配置说明、维护路线图和配置样例。
- 将入口脚本切换到 `argparse`，补充标准 `--help` 输出。
- 补充当前维护状态说明，明确优先整理文档、入口、安全边界和配置项。
- 在维护计划中加入第三方工具目录梳理，便于后续区分源码、配置和运行时产物。
